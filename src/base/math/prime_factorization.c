...
/* global --------------------------------------------------------------------*/
	...
	int16_t	alx_factors_32b [UINT32_MAX][PF_LUT];
/* static --------------------------------------------------------------------*/
...
void	alx_prime_factorization_32b_init	(void)
{
	...
	alx_factors_32b[0][PF_FACTOR]	= -1;
	alx_factors_32b[0][PF_NEXT]	= 0;
	alx_factors_32b[1][PF_FACTOR]	= -1;
	alx_factors_32b[1][PF_NEXT]	= 0;

	for (ptrdiff_t i = 0; i < PRIME_NUMS_QTY_32b; i++) {
		tmp	= alx_prime_32b[i];
		for (ptrdiff_t j = tmp; j < UINT32_MAX; j += tmp) {
			if (!alx_factors_32b[j][PF_NEXT]  &&  !(j % tmp)) {
				alx_factors_32b[j][PF_FACTOR]	= i;
				alx_factors_32b[j][PF_NEXT]	= j / tmp;
			}
		}
	}
}
				...
				bool denominator)
{
	...
	if (!alx_prime_32b[0])
		alx_prime_32b_init();
	if (!alx_factors_32b[(3)][PF_FACTOR])
		alx_prime_factorization_32b_init();

	do {
		if (denominator)
			pf[alx_factors_32b[n][PF_FACTOR]]--;
		else
			pf[alx_factors_32b[n][PF_FACTOR]]++;
		n	= alx_factors_32b[n][PF_NEXT];
	} while (n > 1);
